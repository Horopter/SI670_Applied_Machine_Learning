#!/bin/bash
#
# SLURM Batch Script for Stage 5: x3d Training
#
# Trains x3d model using scaled videos and extracted features.
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage5_x3d.sh

#SBATCH --job-name=fvc_stage5q
...
#SBATCH --output=logs/stage5/stage5q-%j.out
#SBATCH --error=logs/stage5/stage5q-%j.err
...
log "=========================================="
log "STAGE 5: x3d MODEL TRAINING JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
log "=========================================="
log "Starting Stage 5: x3d Model Training"
log "=========================================="
...
STAGE5_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage5/stage5q_${SLURM_JOB_ID:-$$}.log"
...
if "$PYTHON_CMD" -u "$PYTHON_SCRIPT" \
    ...
    if grep -q "CUDA out of memory\|OutOfMemoryError\|OOM" "$LOG_FILE" 2>/dev/null; then
        ...
        log "  Model: $MODEL_TYPE, Frames: $NUM_FRAMES"
        log "  Try reducing FVC_NUM_FRAMES (current: $NUM_FRAMES)"
        log "  Example: FVC_NUM_FRAMES=250 sbatch scripts/slurm_jobs/slurm_stage5q.sh"
    fi
...
log ""
log "============================================================"
log "STAGE 5 (x3d) EXECUTION SUMMARY"
log "============================================================"
