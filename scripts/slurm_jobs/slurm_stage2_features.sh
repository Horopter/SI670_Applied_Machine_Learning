...
#   sbatch --time=6:00:00 scripts/slurm_jobs/slurm_stage2_features.sh
#   sbatch --mem=80G scripts/slurm_jobs/slurm_stage2_features.sh
#
...
if [ "$NODE_SUCCESSES" -eq "$NUM_NODES" ]; then
    ...
    log "Next step: Run Stage 3 with: sbatch scripts/slurm_jobs/slurm_stage3_scaling.sh"
else
    STAGE2_END=$(date +%s)
    STAGE2_DURATION=$((STAGE2_END - STAGE2_START))
