#!/bin/bash
#
# SLURM Batch Script for Stage 5U: two_stream Training
#
# Trains two_stream model (Stage 5U) using scaled videos and extracted features.
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage5u.sh

#SBATCH --job-name=fvc_stage5u
...
#SBATCH --output=logs/stage5/stage5u-%j.out
#SBATCH --error=logs/stage5/stage5u-%j.err
...
log "=========================================="
log "STAGE 5U: two_stream MODEL TRAINING JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
log "=========================================="
log "Starting Stage 5U: two_stream Model Training"
log "=========================================="
...
if "$PYTHON_CMD" -u "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 5U ($MODEL_TYPE) completed successfully in ${STAGE5_DURATION}s ($((${STAGE5_DURATION} / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR/$MODEL_TYPE"
else
    STAGE5_END=$(date +%s)
    STAGE5_DURATION=$((STAGE5_END - STAGE5_START))
    log "✗ ERROR: Stage 5U ($MODEL_TYPE) failed after ${STAGE5_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 5U (two_stream) EXECUTION SUMMARY"
log "============================================================"
