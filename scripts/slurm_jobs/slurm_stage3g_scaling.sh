#!/bin/bash
#
# SLURM Batch Script for Stage 3g: Video Scaling (Frame-based distribution)
#
# Processes videos based on frame count capacity: 2160000 frames (1 day @ 4s/100frames)
# Account: stats_dept2
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage3g_scaling.sh

#SBATCH --job-name=fvc_stage3g_scaling
...
#SBATCH --time=8:00:00
#SBATCH --output=logs/stage3g_scaling-%j.out
#SBATCH --error=logs/stage3g_scaling-%j.err
...
log "=========================================="
log "STAGE 3g: VIDEO SCALING JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
if [ -z "$AUGMENTED_METADATA" ]; then
    ...
    log "  Run Stage 1 first: sbatch scripts/slurm_jobs/slurm_stage1_augmentation.sh"
    exit 1
fi
...
log "=========================================="
log "Starting Stage 3g: Video Scaling"
...
STAGE3_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage3g_scaling_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 3g scaling script..."
...
if "$PYTHON_CMD" "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 3g completed successfully in ${STAGE3_DURATION}s ($((${STAGE3_DURATION} / 60)) minutes)"

    # Run sanity check to verify all scaled videos are in metadata
    log ""
    log "=========================================="
    log "STAGE 3G: SANITY CHECK"
    log "=========================================="
    ...
    STAGE3_END=$(date +%s)
    STAGE3_DURATION=$((STAGE3_END - STAGE3_START))
    log "✗ ERROR: Stage 3g failed after ${STAGE3_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 3g EXECUTION SUMMARY"
log "============================================================"
