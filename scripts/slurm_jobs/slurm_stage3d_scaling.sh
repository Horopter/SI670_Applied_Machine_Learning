#!/bin/bash
#
# SLURM Batch Script for Stage 3d: Video Scaling (Frame-based distribution)
#
# Processes videos based on frame count capacity: 2160000 frames (1 day @ 4s/100frames)
# Account: stats_dept1
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage3d_scaling.sh

#SBATCH --job-name=fvc_stage3d_scaling
...
#SBATCH --time=8:00:00
#SBATCH --output=logs/stage3d_scaling-%j.out
#SBATCH --error=logs/stage3d_scaling-%j.err
...
log "=========================================="
log "STAGE 3d: VIDEO SCALING JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
log "=========================================="
log "Starting Stage 3d: Video Scaling"
...
STAGE3_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage3d_scaling_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 3d scaling script..."
...
if "$PYTHON_CMD" "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 3d completed successfully in ${STAGE3_DURATION}s ($((${STAGE3_DURATION} / 60)) minutes)"

    # Run sanity check to verify all scaled videos are in metadata
    log ""
    log "=========================================="
    log "STAGE 3D: SANITY CHECK"
    log "=========================================="
    ...
    STAGE3_END=$(date +%s)
    STAGE3_DURATION=$((STAGE3_END - STAGE3_START))
    log "✗ ERROR: Stage 3d failed after ${STAGE3_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 3d EXECUTION SUMMARY"
log "============================================================"
