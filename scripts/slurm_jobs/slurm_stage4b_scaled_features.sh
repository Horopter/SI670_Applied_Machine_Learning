#!/bin/bash
#
# SLURM Batch Script for Stage 4b: Scaled Video Feature Extraction
#
# Processes second third of videos (equal distribution)
# Account: si670f25_class (urvim)
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage4b_scaled_features.sh

#SBATCH --job-name=fvc_stage4b_feat
...
#SBATCH --output=logs/stage4b_feat-%j.out
#SBATCH --error=logs/stage4b_feat-%j.err
...
log "=========================================="
log "STAGE 4b: SCALED VIDEO FEATURE EXTRACTION JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
# ============================================================================
# Calculate Video Range (Equal Distribution - Second Third)
# ============================================================================

log "=========================================="
log "Calculating video range for Stage 4b"
log "=========================================="
log "Distribution: Equal video count (second third)"
log "Substage: 4b (2nd of 3 substages)"

# Calculate start and end indices based on equal video distribution
# Substage index: 1 (4b is second substage)
SUBSTAGE_INDEX=1
NUM_SUBSTAGES=3
...
log "=========================================="
log "Starting Stage 4b: Scaled Video Feature Extraction"
...
STAGE4_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage4b_feat_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 4b feature extraction script..."
...
if "$PYTHON_CMD" "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 4b completed successfully in ${STAGE4_DURATION}s ($((${STAGE4_DURATION} / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR"
    log "Next step: Run remaining substages if not all complete"
else
    STAGE4_END=$(date +%s)
    STAGE4_DURATION=$((STAGE4_END - STAGE4_START))
    log "✗ ERROR: Stage 4b failed after ${STAGE4_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 4b EXECUTION SUMMARY"
log "============================================================"
