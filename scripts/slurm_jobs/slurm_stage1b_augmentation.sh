#!/bin/bash
#
# SLURM Batch Script for Stage 1b: Video Augmentation (Range 25-50%)
#
# Generates multiple augmented versions of each video using spatial and temporal
# transformations. Processes second quarter of videos (25-50%).
#
# Usage:
#   # Default: FVC_NUM_AUGMENTATIONS=10, FVC_DELETE_EXISTING=1
#   sbatch scripts/slurm_jobs/slurm_stage1b_augmentation.sh
#   
#   # Or override defaults:
#   FVC_NUM_AUGMENTATIONS=10 FVC_DELETE_EXISTING=1 sbatch scripts/slurm_jobs/slurm_stage1b_augmentation.sh
#
...
#SBATCH --output=logs/stage1b_aug-%j.out
#SBATCH --error=logs/stage1b_aug-%j.err
...
log "=========================================="
log "STAGE 1b: VIDEO AUGMENTATION JOB (Range 25-50%)"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
# Calculate range for 1b (25-50%)
START_IDX=$((TOTAL_VIDEOS / 4))
END_IDX=$((TOTAL_VIDEOS / 2))

log "Stage 1b: Processing videos [$START_IDX, $END_IDX) (second quarter)"
...
log "=========================================="
log "Starting Stage 1b: Video Augmentation (25-50%)"
log "=========================================="
...
STAGE1_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage1b_augmentation_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 1b augmentation script..."
...
if "${STAGE1_CMD[@]}" 2>&1 | tee "$LOG_FILE"; then
    ...
    log "✓ Stage 1b completed successfully in ${STAGE1_DURATION}s ($(($STAGE1_DURATION / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR"
    log "Next step: Run Stage 1a, 1c, 1d in parallel, then Stage 2"
else
    STAGE1_END=$(date +%s)
    STAGE1_DURATION=$((STAGE1_END - STAGE1_START))
    log "✗ ERROR: Stage 1b failed after ${STAGE1_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 1b EXECUTION SUMMARY"
log "============================================================"
