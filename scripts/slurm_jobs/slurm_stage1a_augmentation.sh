#!/bin/bash
#
# SLURM Batch Script for Stage 1a: Video Augmentation (Range 0-25%)
#
# Generates multiple augmented versions of each video using spatial and temporal
# transformations. Processes first quarter of videos (0-25%).
#
# Usage:
#   # Default: FVC_NUM_AUGMENTATIONS=10, FVC_DELETE_EXISTING=1
#   sbatch scripts/slurm_jobs/slurm_stage1a_augmentation.sh
#   
#   # Or override defaults:
#   FVC_NUM_AUGMENTATIONS=10 FVC_DELETE_EXISTING=1 sbatch scripts/slurm_jobs/slurm_stage1a_augmentation.sh
#
...
#SBATCH --job-name=fvc_stage1a_aug
#SBATCH --account=eecs442f25_class
#SBATCH --partition=standard
#SBATCH --time=8:00:00
#SBATCH --mem=80G
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/stage1a_aug-%j.out
#SBATCH --error=logs/stage1a_aug-%j.err
...
log "=========================================="
log "STAGE 1a: VIDEO AUGMENTATION JOB (Range 0-25%)"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
# Calculate range for 1a (0-25%)
START_IDX=0
END_IDX=$((TOTAL_VIDEOS / 4))

log "Stage 1a: Processing videos [$START_IDX, $END_IDX) (first quarter)"
...
log "=========================================="
log "Starting Stage 1a: Video Augmentation (0-25%)"
log "=========================================="
...
STAGE1_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage1a_augmentation_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 1a augmentation script..."
...
if "${STAGE1_CMD[@]}" 2>&1 | tee "$LOG_FILE"; then
    ...
    log "✓ Stage 1a completed successfully in ${STAGE1_DURATION}s ($(($STAGE1_DURATION / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR"
    log "Next step: Run Stage 1b, 1c, 1d in parallel, then Stage 2"
else
    STAGE1_END=$(date +%s)
    STAGE1_DURATION=$((STAGE1_END - STAGE1_START))
    log "✗ ERROR: Stage 1a failed after ${STAGE1_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 1a EXECUTION SUMMARY"
log "============================================================"
