#!/bin/bash
#
# SLURM Batch Script for Stage 1d: Video Augmentation (Range 75-100%)
#
...
#   sbatch scripts/slurm_jobs/slurm_stage1d_augmentation.sh
#   
#   # Or override defaults:
#   FVC_NUM_AUGMENTATIONS=10 FVC_DELETE_EXISTING=0 sbatch scripts/slurm_jobs/slurm_stage1d_augmentation.sh
#   FVC_NUM_AUGMENTATIONS=10 FVC_DELETE_EXISTING=1 sbatch scripts/slurm_jobs/slurm_stage1d_augmentation.sh  # To delete existing
#
...
log "=========================================="
log "STAGE 1d: VIDEO AUGMENTATION JOB (Range 75-100%)"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
# Calculate range for 1d (75-100%)
START_IDX=$((TOTAL_VIDEOS * 3 / 4))
END_IDX=$TOTAL_VIDEOS

log "Stage 1d: Processing videos [$START_IDX, $END_IDX) (fourth quarter)"
...
log "=========================================="
log "Starting Stage 1d: Video Augmentation (75-100%)"
log "=========================================="
...
STAGE1_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage1d_augmentation_${SLURM_JOB_ID:-$$}.log"
...
if "${STAGE1_CMD[@]}" 2>&1 | tee "$LOG_FILE"; then
    ...
    log "✓ Stage 1d completed successfully in ${STAGE1_DURATION}s ($(($STAGE1_DURATION / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR"
    log "Next step: Run Stage 1b, 1c, 1d in parallel, then Stage 2"
else
    STAGE1_END=$(date +%s)
    STAGE1_DURATION=$((STAGE1_END - STAGE1_START))
    log "✗ ERROR: Stage 1d failed after ${STAGE1_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 1d EXECUTION SUMMARY"
log "============================================================"
