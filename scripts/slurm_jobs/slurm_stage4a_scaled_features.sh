#!/bin/bash
#
# SLURM Batch Script for Stage 4a: Scaled Video Feature Extraction
#
# Processes first third of videos (equal distribution)
# Account: si670f25_class (santoshd)
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage4a_scaled_features.sh

#SBATCH --job-name=fvc_stage4a_feat
...
#SBATCH --output=logs/stage4a_feat-%j.out
#SBATCH --error=logs/stage4a_feat-%j.err
...
log "=========================================="
log "STAGE 4a: SCALED VIDEO FEATURE EXTRACTION JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
if [ -z "$SCALED_METADATA" ]; then
    ...
    if [ -f "$RECONSTRUCT_SCRIPT" ]; then
        ...
        log "  Run Stage 3 first: sbatch scripts/slurm_jobs/slurm_stage3_scaling.sh"
        exit 1
    fi
fi
...
# ============================================================================
# Calculate Video Range (Equal Distribution - First Third)
# ============================================================================

log "=========================================="
log "Calculating video range for Stage 4a"
log "=========================================="
log "Distribution: Equal video count (first third)"
log "Substage: 4a (1st of 3 substages)"

# Calculate start and end indices based on equal video distribution
# Substage index: 0 (4a is first substage)
SUBSTAGE_INDEX=0
NUM_SUBSTAGES=3
...
try:
    ...
    end_idx = ($SUBSTAGE_INDEX + 1) * videos_per_substage
...
log "=========================================="
log "Starting Stage 4a: Scaled Video Feature Extraction"
...
STAGE4_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage4a_feat_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 4a feature extraction script..."
...
if "$PYTHON_CMD" "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 4a completed successfully in ${STAGE4_DURATION}s ($((${STAGE4_DURATION} / 60)) minutes)"
    log "Results saved to: $ORIG_DIR/$OUTPUT_DIR"
    log "Next step: Run remaining substages if not all complete"
else
    STAGE4_END=$(date +%s)
    STAGE4_DURATION=$((STAGE4_END - STAGE4_START))
    log "✗ ERROR: Stage 4a failed after ${STAGE4_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 4a EXECUTION SUMMARY"
log "============================================================"
