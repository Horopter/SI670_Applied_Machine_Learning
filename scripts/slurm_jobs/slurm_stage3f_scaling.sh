#!/bin/bash
#
# SLURM Batch Script for Stage 3f: Video Scaling (Frame-based distribution)
#
# Processes videos based on frame count capacity: 2160000 frames (1 day @ 4s/100frames)
# Account: stats_dept2
#
# Usage:
#   sbatch scripts/slurm_jobs/slurm_stage3f_scaling.sh

#SBATCH --job-name=fvc_stage3f_scaling
...
#SBATCH --time=8:00:00
#SBATCH --output=logs/stage3f_scaling-%j.out
#SBATCH --error=logs/stage3f_scaling-%j.err
...
log "=========================================="
log "STAGE 3f: VIDEO SCALING JOB"
log "=========================================="
log "Host:        $(hostname)"
log "Date:        $(date -Is)"
...
log "=========================================="
log "Starting Stage 3f: Video Scaling"
...
STAGE3_START=$(date +%s)
LOG_FILE="$ORIG_DIR/logs/stage3f_scaling_${SLURM_JOB_ID:-$$}.log"
...
log "Running Stage 3f scaling script..."
...
if "$PYTHON_CMD" "$PYTHON_SCRIPT" \
    ...
    log "✓ Stage 3f completed successfully in ${STAGE3_DURATION}s ($((${STAGE3_DURATION} / 60)) minutes)"

    # Run sanity check to verify all scaled videos are in metadata
    log ""
    log "=========================================="
    log "STAGE 3F: SANITY CHECK"
    log "=========================================="
    ...
    STAGE3_END=$(date +%s)
    STAGE3_DURATION=$((STAGE3_END - STAGE3_START))
    log "✗ ERROR: Stage 3f failed after ${STAGE3_DURATION}s"
...
log ""
log "============================================================"
log "STAGE 3f EXECUTION SUMMARY"
log "============================================================"
